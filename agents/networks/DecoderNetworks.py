...
class ConvDecoder84(nn.Module):
    ...
    def build(self, conv_output_shape):
        """
        Build the network bridging the gap between the de-convolutional and linear decoder
        :param conv_output_shape: the shape of the output of the de-convolutional decoder
        """
        self.__compatibility_net = nn.Sequential(
    ...
    def forward(self, x):
        ...
        """
        if self.__compatibility_net is None:
            raise Exception("Error: the decoder was not build().")
        x = self.__lin_net(x)
        x = self.__compatibility_net(x)
